...
const generateTextValue = (fieldName: string): string => {
  const textValues = {
  ...
  const options = textValues[fieldName as keyof typeof textValues] || defaultTexts;
  return options[Math.floor(Math.random() * options.length)];
};

// Generate random number value
const generateNumberValue = (fieldName: string): number => {
  const ranges = {
    Age: { min: 1, max: 100 },
  ...
  const range = ranges[fieldName as keyof typeof ranges] || defaultRange;
...
const generateFields = (count: number) => {
  ...
  for (let i = 0; i < count; i++) {
    const type = fieldTypes[Math.floor(Math.random() * fieldTypes.length)] as 'text' | 'number' | 'date';
...
const generateEntries = (fields: any[], count: number) => {
  const entries = [];
